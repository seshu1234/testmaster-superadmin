...
export default function CreatePlanPage() {
  ...
  const mutation = useMutation({
    mutationFn: (values: z.infer<typeof formSchema>) => {
      return apiClient.post("super-admin/plans", values);
    },
    onSuccess: () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Launch New Tier</h1>
          <p className="text-sm text-muted-foreground">Architect a new offering for the platform ecosystem.</p>
        </div>
      </div>
            ...
            <div className="md:col-span-2 space-y-8">
              <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden bg-card">
                <CardHeader className="bg-muted/30 pb-8 pt-10 px-8">
                  ...
                  <CardTitle className="text-2xl font-black">Core Attributes</CardTitle>
                </CardHeader>
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <Input 
                              ...
                              {...field} 
                              className="h-12 bg-muted/50 border-none rounded-2xl focus-visible:ring-primary shadow-inner px-4 font-mono text-xs"
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input 
                            placeholder="Ideal for large scale institutions with complex needs..." 
                            {...field} 
                            className="h-12 bg-muted/50 border-none rounded-2xl focus-visible:ring-primary shadow-inner px-4"
                          />
                        </FormControl>
                        <FormDescription>Shown to customers during tier selection.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>Usually ~10-20% discount of 12 months.</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
              ...
              <div className="flex justify-end gap-3 pt-4">
                <Button variant="outline" type="button" asChild className="rounded-2xl h-14 px-8 font-bold border-2 transition-all">
                  <Link href="/subscriptions/plans">Cancel</Link>
                </Button>
                <Button 
                  type="submit" 
            ...
            <div className="space-y-8">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className="text-[10px]">-1 for unlimited.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
              ...
              <Card className="border-none shadow-sm rounded-[2rem] overflow-hidden bg-card">
                ...
                <CardContent className="p-6 space-y-5">
                   {[
                     { name: 'ai_grading', label: 'AI Smart Grading', desc: 'Enable automated evaluation engines.' },
                     { name: 'api_access', label: 'Strategic API Access', desc: 'Allow 3rd party system integrations.' },
                     { name: 'custom_branding', label: 'White-labeling', desc: 'Allow custom logs and domain masks.' }
                   ].map((item) => (
                    <FormField
                      key={item.name}
                      control={form.control}
